	...
	*/
	printArray: function(parray, patribute){
		_.each(parray, function(value, index){
			index++;
			colog.log(colog.colorMagenta(index + ': ' + value[patribute]));
		});
	},
	...
	*/
	getConfirmation: function(pname){
		var promise = new RSVP.Promise(function(resolve, reject){
			...
			colog.log(colog.colorMagenta('Do you want to delete: ' + pname));
	...
	*/
	getNumberPrompt: function(prestriction, parray){
		var promise = new RSVP.Promise(function(resolve, reject){
			...
			cancel++;
			colog.log(colog.colorMagenta(cancel + ': Cancel'));
