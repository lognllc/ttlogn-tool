...
var utils = {
	...
	/* perror: message or messages 
	print any error, if the success fails
	*/
	printTTError: function(perror){
		if(_.isArray(perror)){
			_.each(perror, function(value){
				if(value._detail.success === FAIL){
					colog.log(colog.colorRed(value._detail.result));
				}
			});
		}
		else{
			if(perror.success === FAIL){
				colog.log(colog.colorRed(perror.result));
			}
		}
					...
					_.each(projectRepo, function(projectBranch){
						_.each(projectBranch.commits, function(value){
							var validCommit = {
								...
								date = moment(value.committed_date).tz('PST8PDT');
								//console.log(date.format());
							if(value.author.name === pgitName &&
								date.isAfter(limitDate) &&
								FORMAT_HOUR.test(value.message)){
	...
	*/
	...
	/*
	pdescription: name to print 
	returns a bool selection
	*/
	getBoolPrompt: function(pdescription){
		var promise = new RSVP.Promise(function(resolve, reject){
			var self = this;
			...
			prompt.get({
				properties: {
					result: {
						description: pdescription.magenta,
						required: true,
						pattern: TRUE_FALSE,
						message: 'Format: (t|f)'.red
					}
				}
			}, function (err, resultPrompt) {
				if(!err){
					if(resultPrompt.result === 't'){
						resolve(true);
					}
					else{
						resolve(false);
	...
	*/
	...
	getPeriods: function(pdevtype){
		var NON_EXEMPT = 'non_exempt';
		...
		if(pdevtype === 'non_exempt'){
			if((numberOfWeek % 2) === 0){
				period.start = moment().startOf('week') - 1;
				period.end = moment().endOf('week');
			}
			else{
				period.start = moment().startOf('week');
				period.end = moment().endOf('week') + 1;
			}
		}
		else{
			period.start = moment().startOf('month');
			period.end = moment().endOf('month');
		}

	}
	/*
if user[:devtype] == "non_exempt"
			numberOfWeek = Time.now.strftime("%U").to_i + 1
			if numberOfWeek % 2 == 0
				@startOfPeriod = Date.today.at_beginning_of_week - 1.week
				@endOfPeriod = Date.today.at_end_of_week
			else
				@startOfPeriod = Date.today.at_beginning_of_week
				@endOfPeriod = Date.today.at_end_of_week + 1.week
			end
		else
			@startOfPeriod = Date.today.at_beginning_of_month
			@endOfPeriod = Date.today.at_end_of_month
*/
